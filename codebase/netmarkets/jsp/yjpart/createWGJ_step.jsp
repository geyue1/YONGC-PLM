...
<jsp:include page="${mvc:getComponentURL('yjpart.CreateWGJTableBuilder')}"/>

<input type="hidden"  id="yiwgj" name="yiwgj"/>

<script>
    var TABLE_ID = "CreateWGJTableBuilder.table";
  ...
  //js 字符串去左右空格和回车
    ...
    function selectWGJCallback(objects){
        ...
        for (var i = 0, l = pickedObjects.length; i < l; i++) {
        	...
        	list.push(oid);
        	Ext.Ajax.request({
				url : getBaseHref()+"netmarkets/jsp/yjpart/addWGJ3D.jsp",
				params : {"oid":oid},
				success:function(x){
					   if(x.responseText!=null && x.responseText.indexOf("wt.epm.EPMDocument")>-1){
						   list2.push(iGetInnerText(x.responseText));
						   addObjects(list2);  
					   }
				}
		    });
    ...
    function addObjects(oids, tableId) {
        if(!tableId) {
             tableId = "CreateWGJTableBuilder.table";
         }
         var list = [];
