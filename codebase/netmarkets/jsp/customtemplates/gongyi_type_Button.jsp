...
function setPassed(){
	var rows = tableUtils.getTableRows("promotionRequest.promotionItems.table");
	if(!rows){
		rows = tableUtils.getTableRows("changeTask_resultingItems_table");
	
	}
	//var selections = table.getSelectionModel().getSelections();
	var oid = "";
	rows.each(function(record, index, length){
	          oid=oid+ record.get('oid')+",";
    });
	var inputVar = $$('input');
	var vote = "";
	for(var i=0; i<inputVar.length; i++){
            if (inputVar[i].type == 'radio' && inputVar[i].checked) {
            	vote = inputVar[i].value;
            }
	}
	
	Ext.Ajax.request({
		url : getBaseHref()+"netmarkets/jsp/yjpromotion/setVoteValue.jsp",
		params : {"oid":oid,"vote":vote},
		success:function(x){
			// do nothing
		}
	});
